...
//al C:09ae .nmi1
	...
	// Only used labels get to be output for VICE
	if (label->usage && (label->result.flags & MVALUE_DEFINED) && !(label->result.flags & MVALUE_IS_FP))
	{
...
void Label_dump_all_vice(FILE* fd) {
	Tree_dump_forest(Label_forest, ZONE_GLOBAL, dump_one_label_vice, fd);
	if (labeldump_allSections)
	{
		zone_t i;
		for (i=1;i<=zone_max;i++)
