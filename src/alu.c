...
static void get_label_value(zone_t zone) {
	...
	if(pass_count == 0)
		label->usage++;
	if ((label->result.flags & MVALUE_EXISTS) && gParsingMemoryOperand)
	{
...
static inline void parse_decimal_value(void) {// Now GotByte = first digit
	...
	{
		parse_hexadecimal_value();	// MPi: Fix warning
		return;
	}
...
// Operands (values, state changes to ExpectDyadic)
		...
		PUSH_INTOPERAND(CPU_pc.intval, CPU_pc.flags | MVALUE_EXISTS | MVALUE_IS_ADDRESS);
...
int gALU_any_int_flags = 0;
intval_t ALU_any_int(void) {
	result_t	result;

	gALU_any_int_flags = 0;

	if(parse_expression(&result))
		Throw_error(exception_paren_open);
	gALU_any_int_flags = result.flags;
	if((result.flags & MVALUE_EXISTS) == 0)
